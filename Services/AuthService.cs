...
namespace PaieApi.Services
{
    ...
    namespace PaieApi.Services
    {
        public class AuthService
        {
            ...
            public async Task<(bool succes, string message)> DemanderConnexion(string username)
            {
                try
                {
                    var utilisateur = await _mongoDb.Utilisateurs
                    ...
                    if (utilisateur == null)
                    {
                        return (false, "Utilisateur introuvable ou inactif");
                    }

                    if (!utilisateur.TelephoneVerifie)
                    {
                        return (false, "Vous devez d'abord vérifier votre téléphone");
                    }
                    ...
                    var tentativesRecentes = await _mongoDb.TentativesConnexion
                        .CountDocumentsAsync(t =>
                            t.Username == username &&
                            t.DateTentative > DateTime.UtcNow.AddMinutes(-15) &&
                            !t.Succes
                        );
                    ...
                    // Envoyer le code OTP
                    var otpEnvoye = await EnvoyerCodeOTP(username, utilisateur.Telephone, "connexion");
                    ...
                    return (true, $"Code de connexion envoyé au {MasquerTelephone(utilisateur.Telephone)}");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Erreur demande connexion: {ex.Message}");
                    return (false, "Erreur lors de la demande de connexion");
                }
            }
